...
const TradeHistoryApp = () =>
{
    ...
    const [clientHistory, setClientHistory] = useState({instrumentCode: instrumentCode, buyTrades: [], sellTrades: []});
    ...
    {
        const { instrumentCode, clientCode, ownerId} = selectedValues;
        loggerService.logInfo(`Trade History Search Dialog - Search initiated with Instrument Code: ${instrumentCode||"none"}, Client Code: ${clientCode||"none"}, Trader ID: ${ownerId||"none"}`);
    ...
    const fetchTradeHistory = useCallback(async (filterDays, clientCode, instrumentCode, ownerId) =>
    {
        if(clientCode === "" && instrumentCode === "" && ownerId === "")
            return;
        ...
        try
        {
            ...
            const ownerIdSuffix = ownerId !== "" ? `&ownerId=${ownerId}` : `&ownerId=`;

            const url = `http://localhost:20013/orders/history?startTradeDate=${startDate}&endTradeDate=${endDate}${clientSuffix}${instrumentSuffix}${ownerIdSuffix}`;
            ...
            const buyTrades = data.filter(order => order.side === "BUY");
            const sellTrades = data.filter(order => order.side !== "BUY");

            setInstrumentHistory({ instrumentCode, buyTrades, sellTrades });
            setClientHistory({ clientCode, buyTrades, sellTrades });
            setTraderHistory({ ownerId, buyTrades, sellTrades });
    ...
    useEffect(() =>
    {
        fetchTradeHistory(filterDays, clientCode, instrumentCode, ownerId).then(() => loggerService.logInfo("Trade history fetched successfully"));
    }, [filterDays, instrumentCode, clientCode, ownerId, loggerService]);
            ...
            <div className="trade-history-app" style={{width: '100%', height: 'calc(100vh - 67px)', float: 'left', padding: '0px', margin:'45px 0px 0px 0px'}}>
                <TabContext value={selectedTab}>
                    ...
                    <TabPanel value="1" className="client-trade-history">
                        <TradeHistoryGridsComponent rows={clientHistory} historyProperty="clientCode" dataId="client_trade_history" columnDefs={clientColumnDefs} windowId={windowId}/>
                    </TabPanel>)}

                    {selectedTab === "2" && (
                    <TabPanel value="2" className="client-trade-history">
                        <TradeHistoryGridsComponent rows={instrumentHistory} historyProperty="instrumentCode" dataId="instrument_trade_history" columnDefs={instrumentColumnDefs} windowId={windowId}/>
                    </TabPanel>)}

                    {selectedTab === "3" && (
                    <TabPanel value="3" className="client-trade-history">
                        <TradeHistoryGridsComponent rows={traderHistory} historyProperty="ownerId" dataId="trader_trade_history" columnDefs={traderColumnDefs} windowId={windowId}/>
                    </TabPanel>)}
