...
const PriceBreachesGridComponent = () =>
{
    const [breaches, setBreaches] = useState([]);
    const [inboundWorker, setInboundWorker] = useState(null);
    const limitsService = useMemo(() => new LimitsService(), []);
    const loggerService = useMemo(() => new LoggerService(PriceBreachesGridComponent.name), []);
    ...
    useEffect(() =>
    {
        const loadSnapshot = async () =>
        {
            try
            {
                const snapshot = await limitsService.getBreaches("price");
                if (Array.isArray(snapshot))
                    setBreaches(snapshot.map(toBreachRow));
            }
            catch (error)
            {
                loggerService.logError(`Failed to load price breaches: ${error.message}`);
            }
        };
        loadSnapshot();
    ...
    useEffect(() =>
    {
        const webWorker = new Worker(new URL("../workers/price-breach-reader.js", import.meta.url));
    ...
    ([
        ...
        { headerName: 'Arrival Price', field: 'arrivalPrice', width: 130},
        { headerName: 'Difference %', field: 'priceDifferencePercentage', width: 140},
        { headerName: 'Limit %', field: 'priceDifferenceLimit', width: 110},
...
export default PriceBreachesGridComponent;
