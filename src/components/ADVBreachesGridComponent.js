...
const ADVBreachesGridComponent = () =>
{
    const [breaches, setBreaches] = useState([]);
    const [inboundWorker, setInboundWorker] = useState(null);
    const limitsService = useMemo(() => new LimitsService(), []);
    const loggerService = useMemo(() => new LoggerService(ADVBreachesGridComponent.name), []);
    ...
    useEffect(() =>
    {
        const loadSnapshot = async () =>
        {
            try
            {
                const snapshot = await limitsService.getBreaches("adv");
                if (Array.isArray(snapshot))
                    setBreaches(snapshot.map(toBreachRow));
            }
            catch (error)
            {
                loggerService.logError(`Failed to load ADV breaches: ${error.message}`);
            }
        };
        loadSnapshot();
    ...
    useEffect(() =>
    {
        const webWorker = new Worker(new URL("../workers/adv-breach-reader.js", import.meta.url));
    ...
    ([
        ...
        { headerName: 'ADV 20', field: 'adv20', valueFormatter: numberFormatter, width: 130},
        { headerName: 'ADV %', field: 'advPercentage', width: 120, cellStyle: (params) => getPercentageColour(params)},
        { headerName: 'ADV % Limit', field: 'advLimit', width: 130}
    ]), []);
...
export default ADVBreachesGridComponent;
